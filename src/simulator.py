...
All quantities are in *arbitrary simulator units* – see README.
"""
...
def build_fourier_design_matrix(t: np.ndarray, tspan: float, n_modes: int) -> np.ndarray:
...
def power_law_spectrum(
    ...
    """Per-mode variance  ρ_k  for k = 1 .. n_modes.

    ρ_k = A² · (f_k / f_ref)^(-γ) · Δf
    where  f_k = k / T,  Δf = 1 / T.

    Returns shape (n_modes,).
    """
    A = 10.0 ** log10_A
    delta_f = 1.0 / tspan
    fk = np.arange(1, n_modes + 1, dtype=np.float64) / tspan
    rho = A ** 2 * (fk / f_ref) ** (-gamma) * delta_f
